...
| 4 | Refreshed the cyclic replay provenance issues after source-backing `replay_step1_energy.dat` from the canonical Fortran runtime and tightening the remaining blocker to the live runtime mismatch | Round 4 completed the force-plus-energy fixture provenance repair, but the energy row still reproduces through a stdout capture path rather than an appended `energy.dat` row | Keeps `GT-AC1` focused on the constrained-step solver mismatch while preserving the residual fragility of the energy capture path |
...
| 3 | Updated the cyclic replay provenance issues after refreshing `replay_step1_force.dat` from a source-built canonical Fortran replay | Round 3 replaced the stale replay-only force row with a source-backed value, which narrows the mixed-fixture uncertainty to the replay energy row and the live constrained-step mismatch | Keeps `GT-AC1` focused on the remaining source-backed energy-fixture gap and the unresolved C++ replay/output divergence |
