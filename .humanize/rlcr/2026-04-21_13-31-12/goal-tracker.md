...
| The committed cyclic replay-only force-row fixture `replay_step1_force.dat` appears stale relative to a source-built canonical Fortran runtime replay on the same archived cyclic case and trace input | 2 | GT-AC1 | Reconstruct and document the replay-fixture capture path from source, then regenerate or replace the force-row contract before treating it as the cyclic executable-path oracle |
