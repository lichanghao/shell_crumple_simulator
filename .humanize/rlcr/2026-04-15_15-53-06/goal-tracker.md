...
### Plan Version: 1 (Updated: Round 4, 2026-04-15)
...
| task7a: Cyclic BC controller `nCodeLoad=30/31` | AC-9 | in_progress | coding | claude | The 0-based corner-tag mapping fix landed for `nCodeLoad=31`, but the committed gate `E2ECyclicRuntime.CrunchItPostMinimizeFreeStateMatchesCommittedCyclicOracle` is still red at max abs delta about `0.08396`, so the remaining cyclic mismatch is already present in the shared post-`minimize_free` state before the first load increment. |
...
| task7e: Oracle tests for cyclic case and checkpoint | AC-9, AC-10 | pending | coding | claude | The cyclic oracle surface now has two explicit red gates: `E2ECyclicRuntime.CrunchItPostMinimizeFreeStateMatchesCommittedCyclicOracle` fails at max abs delta about `0.08396`, and `E2ECyclicRuntime.CrunchItReplaysCommittedCyclicStepOneTraceDeterministically` fails on the committed replay rows with relative errors about `0.08626`, `0.00666`, `4.1629`, and `1.8587`. Restart parity and incompatible-rank failure coverage are still missing. |
...
| task8c: `AGENT.md` and `document/translation_notes.md` finalization | AC-13 | in_progress | coding | claude | `document/translation_notes.md` now matches the committed cyclic post-free and replay-row gates, but final documentation still depends on closing the remaining AC-7, AC-8, AC-9, AC-10, and AC-11 runtime gaps. |
...
| Source-backed cyclic replay shows the shared post-`minimize_free` state already diverges before load step 1: the instrumented Fortran `post_minimize_free_coords.dat` dump differs from the current C++ step-0 VTU by max abs coordinate delta about `0.08396`, so the remaining cyclic blocker is upstream of `LoadController`. | 2 | AC-7, AC-9, AC-10 | Keep `E2ECyclicRuntime.CrunchItPostMinimizeFreeStateMatchesCommittedCyclicOracle` red until `minimize_free` / translated `lbfgs.f` matches the committed post-free oracle, then rely on first-step cyclic replay and restart parity. |
| The committed cyclic replay-row gate is now numerically enforced and still fails on the first positive-load rows: `E2ECyclicRuntime.CrunchItReplaysCommittedCyclicStepOneTraceDeterministically` reports relative errors about `0.08626` and `0.00666` in `energy.dat`, and about `4.1629` / `1.8587` in `force.dat`. | 4 | AC-9, AC-10 | Keep the replay-row gate red until the shared post-free mismatch and downstream constrained-step / reaction output match the committed replay fixtures. |
