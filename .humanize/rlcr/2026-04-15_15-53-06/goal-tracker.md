...
| task7d: Checkpoint/restart | AC-10 | in_progress | coding | claude | `crunch_it` now ingests `nano_checkpoint.dat` for cyclic runs on rank 0, broadcasts restored `coords` / `eta` / `K0_ref`, resumes from `checkpoint.iload + 1`, writes `checkpoint_nprocs` at end-of-cycle boundaries, and now broadcasts the incompatible-rank status before any rank throws. Executable-path restart parity is still missing, and root-only checkpoint parse/read failures are still not collectively coordinated. |
...
| task8b: Checkpoint compatibility across rank counts | AC-10, AC-11 | pending | coding | claude | Checkpoints now record `checkpoint_nprocs`, and a real `mpirun` regression now covers incompatible-rank rejection, but cross-rank restart acceptance coverage and generic restore-error coordination are still missing. |
...
| Checkpoint restore is only collectively coordinated for the stored-rank mismatch case; other root-only `read_checkpoint()` failures can still strand non-root ranks in the broadcast path. | 10 | AC-10, AC-11 | Turn checkpoint restore into a generic coordinated status/error broadcast that covers parse/label/short-file failures as well as rank mismatch, then add a malformed-checkpoint multi-rank regression test. |
