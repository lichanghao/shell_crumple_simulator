...
### Plan Version: 1 (Updated: Round 2, 2026-04-15)
...
| task7a: Cyclic BC controller `nCodeLoad=30/31` | AC-9 | in_progress | coding | claude | The 0-based corner-tag mapping fix landed for `nCodeLoad=31`, but source-backed replay still shows the remaining cyclic mismatch is already present in the shared post-`minimize_free` state before the first load increment. |
...
| Source-backed cyclic replay shows the shared post-`minimize_free` state already diverges before load step 1: the instrumented Fortran `post_minimize_free_coords.dat` dump differs from the current C++ step-0 VTU by max abs coordinate delta about `0.08396`, so the remaining cyclic blocker is upstream of `LoadController`. | 2 | AC-7, AC-9, AC-10 | Add a committed post-free cyclic oracle fixture and make `minimize_free` / translated `lbfgs.f` match it before relying on first-step cyclic replay or restart parity. |
