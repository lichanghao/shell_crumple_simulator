...
| task7a: Cyclic BC controller `nCodeLoad=30/31` | AC-9 | pending | coding | claude | The executable path still throws for cyclic runtime modes. |
...
| task7d: Checkpoint/restart | AC-10 | in_progress | coding | claude | Archive-backed `nano_checkpoint.dat` read/write support and runtime-state plumbing are now implemented and tested, but executable-path restart ingestion plus cyclic resume behavior are still missing. |
