...
|-------|--------|--------|--------------|
| 0 | Initial tracker seeded from the verified state at the end of the prior RLCR session | The prior loop ended via stop gate after preserving the remaining runtime blockers, source-backed cyclic step-one contract, and completed checkpoint-rejection surface, so the restarted session needs that anchored state rather than a blank template | Keeps the restarted loop aligned with `GT-AC1` through `GT-AC6` without reopening already verified sub-surfaces |
| 1 | Corrected the cyclic replay provenance notes after direct source inspection of `pasapas.f90` showed that the plain canonical Fortran runtime is not a same-trace replay oracle | The canonical cyclic runtime still calls `random_seed()` / `random_number(a)` on every load step and never reads `imperfection_trace.dat`, so the deterministic `replay_step1_*.dat` fixtures cannot be treated as plain-runtime refreshes | Keeps `GT-AC1` anchored to the committed instrumented replay fixtures and avoids debugging against the wrong oracle contract |
| 4 | Refreshed the cyclic replay provenance issues after source-backing `replay_step1_energy.dat` from the canonical Fortran runtime and tightening the remaining blocker to the live runtime mismatch | The previous session completed the force-plus-energy fixture provenance repair, but the energy row still reproduces through a stdout capture path rather than an appended `energy.dat` row | Keeps `GT-AC1` focused on the constrained-step solver mismatch while preserving the residual fragility of the energy capture path |
...
| 3 | Updated the cyclic replay provenance issues after refreshing `replay_step1_force.dat` from a source-built canonical Fortran replay | The prior session replaced the stale replay-only force row with a source-backed value, which narrowed the mixed-fixture uncertainty to the replay energy row and the live constrained-step mismatch | Keeps `GT-AC1` focused on the remaining source-backed energy-fixture gap and the unresolved C++ replay/output divergence |
| 2 | Replaced the old “unspecified instrumented replay path” cyclic provenance issue with the in-repo replay-builder path | `test/cases/tools/build_cyclic_replay_runtime.sh` now rebuilds a replay-only Fortran scratch runtime from source and makes the deterministic `replay_step1_*.dat` contract reproducible in-tree | Keeps `GT-AC1` focused on the remaining live constrained-step mismatch instead of stale provenance ambiguity |
...
| The plain source-built canonical Fortran cyclic runtime is still not a same-trace replay oracle, but the deterministic `replay_step1_*.dat` fixtures are now reproducible in-repo through `test/cases/tools/build_cyclic_replay_runtime.sh`; the remaining blocker is the live C++ constrained-step mismatch against that repaired replay contract | 2 | GT-AC1 | Use the replay-builder path to refresh and validate the deterministic fixtures as needed, then fix the executable-path force / `GNORM` / reaction mismatch against that source-backed contract |
