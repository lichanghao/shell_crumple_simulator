...
| The plain source-built canonical Fortran cyclic runtime is not a same-trace replay oracle because `pasapas.f90` reseeds randomness every load step and never reads `imperfection_trace.dat`, so the committed deterministic `replay_step1_*.dat` fixtures must continue to be treated as coming from the instrumented replay capture path | 1 | GT-AC1 | Preserve the instrumented replay fixtures as the executable-path oracle and debug the live constrained-step mismatch against that committed deterministic contract |
