...
| The committed cyclic step-one replay contract is now source-backed on both `replay_step1_force.dat` and `replay_step1_energy.dat`, so the remaining `GT-AC1` blocker is the live C++ constrained-step mismatch against that repaired contract | 4 | GT-AC1 | Use the refreshed source-backed step-one rows together with the accepted-state-2/3 replay fixtures to fix the executable-path force / `GNORM` / reaction mismatch |
| The refreshed `replay_step1_energy.dat` row is now source-backed, but it is reconstructed from the canonical Fortran runtime's stdout (`Equilibrium energy` plus the last `CRITC`) rather than an appended `energy.dat` row, so its reproduction path remains more fragile than the force-row path | 4 | GT-AC1 | Preserve the documented stdout capture path while prioritizing the live constrained-step runtime fix that is still blocking cyclic executable-path parity |
