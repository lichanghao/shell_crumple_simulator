...
Solution: Remove the `gnorm_ < eps_` convergence check from the iflag=1 branch. Only check it in the iflag=0 branch (after a full curvature-accepted step). Keep only `icall > max_eval_` as the safety guard in the iflag=1 branch.
Constraints: This applies specifically to Nocedal-1980 LBFGS + MCSRCH as translated from lbfgs.f. The convergence criterion in this codebase is `GNORM/100 + |ddX|/XNORM0 <= EPS`, evaluated inside lbfgs_step at the end of the iflag=0 pass.
Validation Evidence: After fix, step-0 energy = 7.68e-25 (correct fully-relaxed state). E2ECompression.EnergyAndForceMatchOracle passes: 51 energy rows within 1e-4 relative, 50 force rows within 1e-3 relative; 72/72 unit + 19/19 integration tests pass.
Source Rounds: 31
