...
- Full-state accepted-step fixtures now pin down the earliest committed divergence boundary more precisely: accepted state 2 still matches the same-trace Fortran replay within tight coordinate tolerance, while accepted state 3 is already materially divergent in the full coordinate state even though `eta` still matches. That means the cyclic replay mismatch begins almost immediately after the first accepted constrained update, not only later in the accepted-step head.
