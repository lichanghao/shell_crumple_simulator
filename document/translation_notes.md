...
- Fresh restarted-loop probing against a source-built canonical Fortran runtime (`mpif77 -fallow-argument-mismatch` build of `grapheneCompressionOriginVersion/`) suggests the committed replay-only force-row fixture is stale or at least captured through a different path than the archived runtime contract. On the same cyclic replay case and `replay_step1_trace.dat`, the source-built Fortran `force.dat` step-one row was approximately `-0.000064340, 0.001056593`, which matches the archived `simulator_run/force.dat` row much more closely than the committed replay-only fixture `replay_step1_force.dat` (`0.000022279, 0.001250697`). That means the live cyclic blocker is now split between the executable-path C++ vs source-built-Fortran runtime mismatch and the stale provenance of the replay-only force-row fixture.
