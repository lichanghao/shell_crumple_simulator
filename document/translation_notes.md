...
| Constitutive kernels and archived element-state coverage | translated and verified | Brenner, principal, exponential, archived constitutive fixtures, archived element-energy fixtures, and the first-step exact-state gates are green after regenerating the stale compression-dependent Fortran fixtures from the refreshed `np1` archive. |
...
- The committed executable-path regression now reads the archived `energy.dat` and `force.dat` directly instead of duplicating stale hard-coded step-1 literals. After refreshing the `np1` runtime archive from a fresh Fortran `crunch_it` run at commit `7d3f77f`, the archive-backed compression trajectory checks, post-free parity, first constrained-step replay, and archived VTU-vs-row checks are green again.
...
- The remaining divergence is no longer on the compression first-step exact-state or archive-backed runtime surface. The reconstructed element-83 replay-only gate, the post-`minimize_free` compression replay gate, the archive-backed compression runtime checks, and the refreshed flat/Brenner `ElementEnergy` oracle fixtures are green in the current tree.
- That means the live AC-7/AC-9 blocker is now concentrated on the cyclic executable path (`pasapas` / constrained minimization / reaction output), not on the compression first-step exact-state kernel oracle surface.
...
- Fresh restarted-loop probing showed that a plain source-built canonical Fortran runtime is **not** a valid same-trace replay oracle because `pasapas.f90` still calls `random_seed()` and `random_number(a)` each cyclic load step and never reads `imperfection_trace.dat`. To make the deterministic replay contract reproducible, the repository now carries `test/cases/tools/build_cyclic_replay_runtime.sh`, which builds a replay-only scratch copy of the canonical Fortran runtime with `imperfection_trace.dat` wired into the copied `pasapas.f90`. A fresh `crunch_it_replay` run from that helper reproduced the committed step-one replay rows (`3.04453588e-04` energy and `0.000022279 / 0.001250697` force), so the cyclic replay fixtures are again source-backed. The remaining cyclic blocker is therefore the constrained-step C++ runtime mismatch against that repaired deterministic replay contract, especially on `GNORM` and reaction output.
...
- A compact committed fixture now covers the first 20 accepted cyclic L-BFGS states (`replay_step1_accepted_lbfgs_head.dat`). The corresponding regression shows the current C++ path still matches those early accepted-step states within tight coordinate tolerances and monitor-compatible energy/criterion tolerances. The remaining divergence therefore starts later in the constrained solve than the first 20 accepted updates.
- Full-state accepted-step fixtures now pin down the earliest committed divergence boundary exactly: accepted states 1 and 2 still match the same-trace Fortran replay within tight coordinate tolerance, while accepted state 3 is already materially divergent in the full coordinate state even though `eta` still matches. That means the cyclic replay mismatch begins at the third accepted constrained update, not only later in the accepted-step head.
- The accepted-state-2 reduced optimization variables sharpen that boundary further: `x_free` still matches the same-trace Fortran replay at accepted state 2 within tight tolerance, but `g_free` is already materially divergent there. That points the next solver fix at the free-gradient / force path rather than at the accepted-step coordinate update itself.
