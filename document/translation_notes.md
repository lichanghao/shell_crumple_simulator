...
- The remaining divergence shows up on the very first constrained-step energy evaluation, before the outer L-BFGS trajectory meaningfully branches. On the reconciled authoritative same-trace Fortran replay for reconstructed element 83, both the replay-only gate and the standalone exact-state unit gate are now green. The remaining analytical blocker is the broader archived constitutive/kernel surface, which is still red on `ElementState.MatchesArchivedCompressionSimulatorOracleFixtures`, `ElementEnergy.MatchesArchivedCompressionSimulatorOracleFixtures`, and `ElementEnergy.FElemMatchesFortranOracle`.
