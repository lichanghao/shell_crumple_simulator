...
- Fresh restarted-loop probing against a source-built canonical Fortran runtime (`mpif77 -fallow-argument-mismatch` build of `grapheneCompressionOriginVersion/`) showed that the old replay-only force-row fixture was stale relative to the archived runtime contract. On the same cyclic replay case and `replay_step1_trace.dat`, the source-built Fortran step-one `force.dat` row was `-0.000000985, 0.001052467`, and the source-backed step-one energy contract came from the printed `Equilibrium energy: 2.7438748296784488E-004` line plus the last printed `CRITC` before the step-2 banner (`9.646D-06`). The committed `replay_step1_force.dat` and `replay_step1_energy.dat` fixtures have now been refreshed to those source-backed values. The cyclic blocker is therefore narrowed to the executable-path C++ vs source-built-Fortran runtime mismatch, not just stale replay-only fixture provenance.
