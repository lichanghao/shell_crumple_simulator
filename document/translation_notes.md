...
- Fresh restarted-loop probing against a source-built canonical Fortran runtime (`mpif77 -fallow-argument-mismatch` build of `grapheneCompressionOriginVersion/`) showed that the old replay-only force-row fixture was stale relative to the archived runtime contract. On the same cyclic replay case and `replay_step1_trace.dat`, the source-built Fortran `force.dat` step-one row was approximately `-0.000064340, 0.001056593`, which matches the archived `simulator_run/force.dat` row much more closely than the old replay-only fixture. The committed `replay_step1_force.dat` fixture has now been refreshed to that source-backed row. The cyclic blocker is therefore still split between the executable-path C++ vs source-built-Fortran runtime mismatch and the still-unsettled source-backed capture path for `replay_step1_energy.dat`.
