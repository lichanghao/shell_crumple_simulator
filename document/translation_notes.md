...
| Constitutive kernels and archived element-state coverage | translated and partially verified | Brenner, principal, exponential, archived constitutive fixtures, and first-step exact-state gates are committed; the full current `ElementEnergy` oracle surface still has red unit tests that must be resolved before this row can return to verified. |
...
- The remaining divergence is no longer dominated by the first-step exact-state oracle surface, but the full archived constitutive/kernel surface is not completely clean either. The reconstructed element-83 replay-only and standalone exact-state gates are green, while the full current unit suite still reports red `ElementEnergy.FlagNumDiffStressesMatchFortranOracle` and `ElementEnergy.BrennerMaterialMatchesFortranOracle` coverage. The remaining AC-7 blocker is still centered on the executable-path step-one energy / `GNORM` / reaction-force output, but the element-energy verification surface should not be treated as fully closed yet.
