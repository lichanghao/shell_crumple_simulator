...
- Fresh restarted-loop probing against a plain source-built canonical Fortran runtime (`mpif77 -fallow-argument-mismatch` build of `grapheneCompressionOriginVersion/`) showed that this runtime is **not** a valid same-trace replay oracle: `pasapas.f90` still calls `random_seed()` and `random_number(a)` each cyclic load step and never reads `imperfection_trace.dat`. That means the deterministic cyclic replay fixtures must continue to be treated as coming from a separate instrumented replay capture path, not from an unmodified canonical runtime rebuild. The cyclic blocker therefore remains the constrained-step runtime mismatch against the committed deterministic replay fixtures, especially on `GNORM` and reaction output.
