...
- A new generated-state consistency check `E2ECyclicRuntime.GeneratedStepOneVtuMatchesGeneratedEnergyAndReactionRows` is green. That means the current cyclic step-one VTU, `energy.dat`, and `force.dat` rows are internally consistent with each other under the C++ solver, so the remaining red surface is the replay-oracle parity itself rather than an inconsistency between the emitted files.
