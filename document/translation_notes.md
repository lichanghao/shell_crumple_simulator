...
- The committed executable-path regression now reads the archived `energy.dat` and `force.dat` directly instead of duplicating stale hard-coded step-1 literals. That removed one source of oracle drift, but the committed archive and the disposable same-trace replay still disagree on the final step-1 target row, so AC-7 still has a source-of-truth problem to resolve.
...
- The remaining divergence shows up on the very first constrained-step energy evaluation, before the outer L-BFGS trajectory meaningfully branches. On a reconstructed first-evaluation state for element 83, a temporary same-trace Fortran oracle gives `W_elem = 1.51680195367486864e-01` and converged inner shifts `eta ≈ [1.7059377382830062e-04, -1.3871993792757202e-04]` and `[1.1767640905118179e-03, -1.5551411794653993e-03]`, while the current C++ analytical path gives `W_elem = 1.45393670059607322e-01` and materially different `eta`.
