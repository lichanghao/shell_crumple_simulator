...
| Constitutive kernels and archived element-state coverage | translated and verified | Brenner, principal, exponential, archived constitutive fixtures, archived element-energy fixtures, and the first-step exact-state gates are now green. |
...
- The remaining divergence no longer shows up in the archived constitutive/kernel gates. On the reconciled authoritative same-trace Fortran replay for reconstructed element 83, both the replay-only gate and the standalone exact-state unit gate are green, and the archived `ElementState` / `ElementEnergy` oracle fixtures are green as well. The remaining AC-7 blocker is now the executable-path step-one energy / `GNORM` / reaction-force output.
...
- The cyclic `nCodeLoad=31` runtime now has the same kind of replay-vs-archive split. Committed cyclic step-one replay fixtures (`test/cases/graphene_cyclic_crumple/replay_step1_*.dat`) provide a deterministic source-backed contract, but the new committed cyclic post-free oracle shows the shared `minimize_free` path is still red there: `E2ECyclicRuntime.CrunchItPostMinimizeFreeStateMatchesCommittedCyclicOracle` currently fails at about `0.08396` max absolute coordinate error before the first cyclic load increment.
