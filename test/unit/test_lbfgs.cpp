...
TEST(LbfgsConvergesOnQuadratic, SmallDimension) {
    ...
    const int flag = solver.minimize(x, xnorm0, quadratic_callback);
...
TEST(LbfgsResetClearsHistory, TwoSequentialMinimizations) {
    ...
    int flag1 = solver.minimize(x1, xnorm0, quadratic_callback);
    ...
    int flag2 = solver.minimize(x2, xnorm0, quadratic_callback);
...
TEST(LbfgsConvergesOnRosenbrock, TwoDimensional) {
    ...
    fce::LbfgsSolver solver(10, 1.0e-8, 1.0e-12, 20000);
    const int flag = solver.minimize(x, xnorm0, rosenbrock_callback);
