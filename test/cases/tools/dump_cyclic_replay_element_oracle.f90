program dump_cyclic_replay_element_oracle
  use data_mat
  use data_mesh
  use data_vector2
  use data_vector3
  ...
  interface
    ...
    subroutine ghost_nodes(meshh, xx)
      ...
      implicit real(8) (a - h, o - z)
      implicit integer*4(i - n)
      type(mesh), intent(in) :: meshh
      real(8), intent(inout) :: xx(3*(meshh%numnods + meshh%nedge))
    end subroutine ghost_nodes
  end interface

  character(len=512) :: case_dir, coords_path, eta_path, out_path
  ...
  real(8) :: eta_final(2), eta_in(2), W, dW(6), W_, dW_(6), S_n(3), S_m(3)
  real(8) :: f_elem(12, 3), W_elem
  logical :: flag_num_diff
  real(8), parameter :: crit = 1.0d-8, h = 1.0d-8

  call get_command_argument(1, case_dir)
  call get_command_argument(2, coords_path)
  call get_command_argument(3, eta_path)
  call get_command_argument(4, out_path)
  if ((len_trim(case_dir) == 0) .or. (len_trim(coords_path) == 0) .or. &
      (len_trim(eta_path) == 0) .or. (len_trim(out_path) == 0)) then
    stop "usage: dump_cyclic_replay_element_oracle <case-dir> <coords-dump> <eta-dump> <out-path>"
  end if

  call read_dims(trim(case_dir) // "/nano_dims.dat", numele, numnods, nedge, ngauss)
  allocate(neigh_vert(12, numele))
  allocate(x0(numnods + nedge, 3))
  allocate(eta(2, numele*ngauss))
  ...
  mesh0%numnods = numnods
  mesh0%nedge = nedge
  call read_mesh_data(trim(case_dir) // "/nano_Mesh.dat", numele, nedge, neigh_vert, mesh0%nghost_tab)
  call fill_ghost_coords(mesh0, x0)
...
contains
  ...
  subroutine read_dims(path, numele, numnods, nedge, ngauss)
    character(len=*), intent(in) :: path
    integer, intent(out) :: numele, numnods, nedge, ngauss
  ...
  subroutine read_zero(path, numele, F0)
    ...
    do jelem = 1, numele
      read(12, *) dummy
      read(12, *) F0(1, 1, jelem), F0(1, 2, jelem)
      read(12, *) F0(2, 1, jelem), F0(2, 2, jelem)
    end do
    close(12)
  end subroutine read_zero

  subroutine read_coord_dump(path, numnods, x0)
    character(len=*), intent(in) :: path
  ...
  subroutine read_eta_dump(path, numele, ngauss, eta)
    character(len=*), intent(in) :: path
    integer, intent(in) :: numele, ngauss
    real(8), intent(out) :: eta(2, numele*ngauss)
    integer :: row, ielem, igauss, idx
    open(unit=16, file=path, status="old", action="read")
    do row = 1, numele*ngauss
      read(16, *) ielem, igauss, eta(1, row), eta(2, row)
      idx = (ielem - 1)*ngauss + igauss
      if (idx /= row) stop "eta dump ordering mismatch"
    end do
    close(16)
  end subroutine read_eta_dump

  subroutine read_mesh_data(path, numele, nedge, neigh_vert, nghost_tab)
    character(len=*), intent(in) :: path
    integer, intent(in) :: numele, nedge
    integer, intent(out) :: neigh_vert(12, numele)
    integer, intent(out) :: nghost_tab(nedge, 3)
    character(len=512) :: line
    integer :: jelem, jj, a, b, c, d, code_bc_dummy(3), neigh_elem_dummy, iedge
    ...
    do jelem = 1, numele
      ...
      read(14, *) a
      do jj = 1, 12
        read(14, *) neigh_elem_dummy, neigh_vert(jj, jelem)
      end do
      read(14, *) code_bc_dummy(1), code_bc_dummy(2), code_bc_dummy(3)
    end do
    call skip_to_label(14, "nghost_tab")
    do iedge = 1, nedge
      read(14, *) nghost_tab(iedge, 1), nghost_tab(iedge, 2), nghost_tab(iedge, 3)
    end do
    close(14)
  end subroutine read_mesh_data

  subroutine fill_ghost_coords(mesh0, x0)
    type(mesh), intent(in) :: mesh0
    real(8), intent(inout) :: x0(:, :)
    real(8), allocatable :: flat(:)
    integer :: inode

    allocate(flat(3*(mesh0%numnods + mesh0%nedge)))
    do inode = 1, mesh0%numnods + mesh0%nedge
      flat(3*inode - 2:3*inode) = x0(inode, :)
    end do
    call ghost_nodes(mesh0, flat)
    do inode = 1, mesh0%numnods + mesh0%nedge
      x0(inode, :) = flat(3*inode - 2:3*inode)
    end do
    deallocate(flat)
  end subroutine fill_ghost_coords
...
end program dump_cyclic_replay_element_oracle
